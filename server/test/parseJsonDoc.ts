...
function parseDoc(doc: Array<SpriteComment>) {
  let spriteTypes = {};
  
  doc.forEach(function(comment) {
    let tags: Array<TagLine> = comment.tags;

    // Check if it's a class comment
    let classTag: TagLine = tags.find(tagLine => tagLine.tag == 'class');
    if (classTag) {
      let spriteTypeName: string = classTag.name;
      // Set SpriteType name and description
      let spriteType: SpriteType = { name: spriteTypeName, descr: comment.description, private: false, properties: [], events: [], methods: [] };
      spriteTypes[spriteTypeName] = spriteType;
      
      // Fill SpriteType properties and events
      tags.forEach((tag) => {
        if (tag.tag == 'property') {
          let property: Property = { name: tag.name, type: tag.type, descr: tag.description, defVal: tag.default };
          spriteType.properties.push(property);
        } else if (tag.tag == 'event') {
          let event: SpriteEvent = { name: tag.name, descr: tag.description };
      ...
      let className = tags.find(tagLine => tagLine.tag == 'memberof').name;
      let method: Method = {name: '', descr: comment.description, params: [], return: {type: '', descr: ''}};
